...
Performance was an issue throughout this project.  Long processing times limited the amount of experimentation.  The final product is still far too slow for any real time processing, averaging only 1 and a half frames per second.   Thoughts to speed it up is to switch to OpenCV's SVM implementation which may be faster (assuming it utilzies the GPU).   My other idea was to switch from SVM to a simple neural net (1 layer with softmax) to process the feature vector and utilize TensorFlow and GPU processing.

I was significantly delayed by a bad assumption on my part that _sklearn_'s `SVC(kernel='linear')` was equivalent to `LinearSVC()`.   My early attempt at this project utilized `LinearSVC()` and was getting close to 2 frames per second processing.   I then tried to "improve" my project with automatic hyperperameter tuning, including the kernel, so I switched to using the `SVC()` class that takes the kernel as a parameter.  I found that my "auto-tuned" implementation was getting less than a frame per second, and switching back to my original params made no difference.   After may hours of paying with parameters to try and figure out the slowdown, I finally figured out it was the switch from `LinearSVC()` to `SVC(kernel='linear')`.   In the `VehicleClassification.train()` method, I added logic to see if the user passed in a kernel of 'linear' and create a `LinearSVC()` in that scenario.

If I were to do this project again, I don't think I would use "HOG"/"SVM".  I would like to experiment with more recent solutions for "object detection with localization" such as [yolo](https://github.com/pjreddie/darknet/wiki/YOLO:-Real-Time-Object-Detection) and [ssd](https://github.com/weiliu89/caffe/tree/ssd).
